...
sub _fetchFormFieldValue {
    ...
    if ( $field eq 'text' ) { return $meta->text; }
...
sub _MULTITOPICSAVEINPUT {
    ...
    # Special field text is the topic text and must always be textarea
    $type = 'textarea' if ( $field eq 'text'); 
    ...
    foreach my $topickey ( keys %parameters ) {
        ...
        foreach my $fieldName ( keys %{$parameters{$topickey}} ) {
            ...
            if ( $fieldName eq 'text' ) {
                ...
                # Remove carriage returns because the Store also does that
                # The normal storage of a topic always leaves at least one new line 
                $value =~ s/\r//g;
                $value = "\n" if ( $value eq '' );
            }
            else {
